...
매 비파울 샷 끝나면, 큐 공이 **가장 가까운 빨간 공** 으로부터의 거리 d_min 에 대한 가우시안 보너스:
...
#### 병모의 `gentle_shot` 과 뭐가 다른가? — 4가지 차이

| 측면 | gentle_shot (병모) | setup_shaping (우리) |
|---|---|---|
| **언제 발동** | **득점 샷 직후만** (sparse) | **매 비파울 샷마다** (dense) |
| **Target ball** | 그 샷에서 두 번째로 맞춘 red | 그냥 가장 가까운 red (둘 중 어느 거든) |
| **함수 모양** | Gaussian peak (d_target=0.2m 일 때 max) | Exponential decay (d=0 일 때 max) |
| **크기 (max α)** | 0.2 (큼) | 0.05 (작음) |

#### 시각화 (1 샷 후 cue ball 의 거리 → bonus)

```
gentle_shot (병모, 득점 후만):           setup_shaping (우리, 매 샷):
                                         
bonus                                    bonus
0.2 |    ███   peak at 0.2m              0.05|██
    |   █   █                                |  ██
    |  █     █                               |    ██
0.0 |█_________█___                       0.0|______██████___
    0  0.2  0.4   d(m)                       0   0.2  0.4   d(m)
       ▲                                     ▲
    d_target                              d=0 (붙어있음)
```

- gentle: "공이 두 번째 red 와 *적당히* (0.2m) 떨어진 위치에 멈춰라"
- setup_shaping: "공이 어떤 red 든 *가까운 곳에* 멈춰라 (가까울수록 좋음)"

#### 같은 상황에서 두 보상의 비교

상황 1: 득점 + 두 번째 red 와 거리 0.2m, 가장 가까운 red 와 거리 0.2m
- gentle_shot: **0.2** (peak)
- setup_shaping: 0.05 · exp(-0.2/0.3) = 0.026

상황 2: 득점 + 두 번째 red 와 거리 0.5m, 가장 가까운 red 와 거리 0.1m
- gentle_shot: **0.012** (벨 곡선의 꼬리)
- setup_shaping: 0.05 · exp(-0.1/0.3) = **0.036**

상황 3: 미스 + 가장 가까운 red 와 거리 0.1m
- gentle_shot: **0** (득점 안 했으니 발동 안 함)
- setup_shaping: **0.036** (그래도 보너스 줌)

상황 4: 미스 + 모든 red 와 거리 1.0m (완전 헛샷)
- gentle_shot: 0
- setup_shaping: 0.05 · exp(-1.0/0.3) = 0.002 (거의 0)

#### 왜 dense 가 필요했나

병모의 `gentle_shot` 만 있을 때:
- 학습 초기 정책은 거의 득점 못 함 (sparse score)
- 그러면 `gentle_shot` 도 거의 발동 안 함 → **신호 자체가 매우 드묾**
- agent 가 "다음 샷 setup" 의 개념을 학습할 데이터 부족

우리 `setup_shaping` 추가 시:
- 미스 / 득점 무관 매 샷마다 "큐가 red 근처에 멈췄는지" 측정 → 작은 신호
- 학습 초기에도 미스 샷에서 신호 받음 → "어쨌든 red 근처로 가는 게 낫다" 학습
- 결과: 동일 wall 에서 mean 0.575 → 0.885 (+54%)

#### 둘 사이의 관계

본 우리 SOTA 정책은 **둘 다 켜져 있음** (병모의 gentle_shot 유지 + 우리 setup_shaping 추가). 보완적:
...
파울?  → -foul_penalty (음수)
   ↓
미스 (점수 안 남)  → 0
                  + setup_shaping (가까운 red 거리)   ← 우리 추가
   ↓
득점 → 1
     + setup_shaping (가까운 red 거리)               ← 우리 추가  
     + gentle_shot (두 번째 red 의 ideal distance)  ← 병모
```
